...
const userArticles = computed(() => articleStore.currentUserArticles)
...
async function loadInitialData() {
  ...
  const temp2 = await articleStore.fetchUserInformation(route.params.username)
  console.log('CurrentUsersInformation: ', temp2)
    ...
    <div class="h-fit m-5 bg-white rounded-lg overflow-hidden">
      ...
      >
        <img
          ...
          alt=""
          class="rounded-full w-40 h-40 border-8 border-blue-700"
        />
      </div>
      ...
      <div class="flex flex-row justify-center p-10 gap-3">
        <span v-if="userInformation.github_username" class="flex flex-row">
          <i class="fa-brands fa-github text-xl"></i>
          <p>{{ userInformation.github_username }}</p>
        </span>
        <span v-if="userInformation.twitter_username" class="flex flex-row">
